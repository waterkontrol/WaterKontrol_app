...
async function sendCredentialsToDevice(e) {
    ...
    showMessage("info", "📡 Enviando credenciales Wi-Fi al dispositivo (IP 192.168.4.1)... Asegúrate de estar conectado al Wi-Fi del dispositivo.", "blue");
    ...
    try {
        ...
        if (!response.ok) {
             showMessage("error", `❌ Error en la API local del dispositivo (Status: ${response.status}). ¿Estás conectado al Wi-Fi WaterKontrol-AP?`, "red");
             submitButton.disabled = false;
             return;
        }

        showMessage("info", "✅ Credenciales aceptadas por el dispositivo. Registrando en la plataforma WaterKontrol...", "blue");
        ...
        if (registerResponse.ok) {
            showMessage("success", "🎉 ¡Dispositivo configurado y registrado en la plataforma WaterKontrol! Redirigiendo...", "green");
            setTimeout(() => {
                window.location.href = '/app.html';
            }, 8000);
        } else {
             const errorData = await registerResponse.json().catch(() => ({ message: 'Error desconocido.' }));
             showMessage("error", `❌ Error al registrar en la plataforma (Status: ${registerResponse.status}): ${errorData.message || registerResponse.statusText}`, "red");
        }

    } catch (error) {
        showMessage("error", `❌ Error de conexión: ${error.message}. Asegúrate de que tu celular/PC esté **conectado a la red Wi-Fi temporal del dispositivo** (ej: WaterKontrol-AP) para enviar las credenciales.`, "red");
    }
    submitButton.disabled = false;
}
